...
print("Welcome! I can help you with rooftop solar adoption and grid flexibility.")
print("How can I assist you today? (Type 'quit' to exit)")

chat_history = [] # Keep chat history across turns
# Initialize a persistent state dictionary
persistent_state = {
    'chat_history': [],
    'user_info': {},
    'solar_options': [],
...
while True:
    ...
    # Update the input for the current turn in the persistent state
    persistent_state['input'] = user_input
    # Append user input to chat history immediately
    persistent_state['chat_history'].append(HumanMessage(content=user_input))


    # Run the graph, starting from handle_user_input for each new user input
    # The state is passed and modified throughout the nodes within one turn's execution
    # The handle_user_input node now sets the initial 'current_stage'
    # try:
    # We are passing the entire persistent_state and expecting it to be modified
    # within the graph run for this turn.
    print (persistent_state)
    final_state = app.invoke(persistent_state) # Use invoke for a single pass through the graph for this turn

    # Update the persistent state with the result of the graph run
    persistent_state = final_state.copy()
    # Clear turn-specific input and messages
    persistent_state['input'] = None
    persistent_state['tool_output'] = None
    persistent_state['latest_tool_output_summary'] = None
    # Error message should be handled by the agent's response in case of error,
    # but keep it in state if needed for subsequent error handling logic.
    # For now, let's clear it assuming the agent addressed it.
    # persistent_state['error_message'] = None


    # The final message in the chat_history should be the agent's response for this turn
    # Find the last AIMessage in the history for the response
    ai_response = None
    # Iterate backwards to find the last AI message generated this turn
    # This is tricky with invoke vs stream. With invoke, the history contains all messages of the turn.
    # The last message added is the agent's response.
    if persistent_state['chat_history'] and isinstance(persistent_state['chat_history'][-1], AIMessage):
            ai_response = persistent_state['chat_history'][-1]
    elif len(persistent_state['chat_history']) > 1 and isinstance(persistent_state['chat_history'][-2], AIMessage):
            # Sometimes the last message could be a ToolMessage added after the AI message
            ai_response = persistent_state['chat_history'][-2]


    if ai_response and ai_response.content.strip():
            print(f"Agent: {ai_response.content}")
    # If no AI message or empty, maybe there was an error handled internally without a clear message
    elif persistent_state.get('error_message'):
            print(f"Agent: An error occurred: {persistent_state['error_message']}")
            # Optionally transition to a specific error handling state or message outside the main loop
            # For this demo, the agent node handles the error message within the graph.
    else:
            print("Agent: (No explicit response generated this turn)") # Should not happen if agent is working correctly


    # except Exception as e:
        # print(f"An unexpected error occurred during graph execution: {e}")
        # # Add error to history
        # persistent_state['chat_history'].append(AIMessage(content=f"An internal error occurred: {e}. Please try again."))
        # print(f"Agent: An internal error occurred. Please try again.")
        # # Reset stage to allow restarting
        # persistent_state['current_stage'] = 'welcome'
        # persistent_state['error_message'] = str(e)
